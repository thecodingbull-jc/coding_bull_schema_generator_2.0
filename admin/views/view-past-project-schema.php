...
<div class="schema-generator-section" style="display:flex; gap:32px; height:1000px;">
    <div style="display:flex; flex-direction:column; gap:20px; width:50%;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <div style="display:flex; gap:16px;">
                <button id="schema-generator-past-project-save-btn" class="button button-primary">Save</button>
            </div>
        </div>
    ...
    <div style="width:50%;">
            ...
            <textarea id="schema-generator-past-project-schema-result" style="display:block; width:100%; height:calc(100% - 18px); background:white; margin-top:20px; border-radius:20px; padding:16px;" readonly>
...
<script>
    jQuery(document).ready(function($) {
        ...
        $.ajax({
            ...
            method: 'POST',
            data: {
                action: 'past_project_generate_schema', // AJAX action name
            ...
            success: function(response) {
                if(response.success){
                    ...
                    response.data.schema.forEach(element => {
                        ...
                        if(done == 0 ){
                            $('#schema-generator-past-project-schema-result').val(JSON.stringify(JSON.parse(element),null,2));
                            done++;
                        }
                    });
        ...
        $.ajax({
            ...
            data: {
                ...
                page: "past-project",                  // Page value to query
                nonce: schemaAjax.nonce // Security nonce
            },
            success: function(response) {
                if(response.success){
        ...
        //handle save button
        $('#schema-generator-past-project-save-btn').on('click', function(e) {
            e.preventDefault();

            let page = 'past-project';
            let data = [];
